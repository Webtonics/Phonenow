  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await adminService.getOrders({
        page,
        per_page: 10,
        type: typeFilter as 'phone_number' | 'esim' | undefined || undefined,
        status: statusFilter || undefined,
      });
      if (response.success) {
        // Filter by search on client side (order_number, user name, email)
                    ...
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full ${
                          order.type === 'phone_number' ? 'bg-primary-100 text-primary-700' :
                          order.type === 'esim' ? 'bg-purple-100 text-purple-700' :
                          'bg-gray-100 text-gray-700'
                        }`}>
                          {getTypeIcon(order.type)}
                          {order.type === 'phone_number' ? 'Phone' : order.type === 'esim' ? 'eSIM' : order.type}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-medium">₦{order.amount_paid?.toLocaleString() || 0}</td>
                      <td className="px-4 py-3">
              ...
              {orders.map((order) => (
                <div key={order.id} className="p-4">
                  ...
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="font-medium text-gray-900">₦{order.amount_paid?.toLocaleString() || 0}</span>
                  </div>
                  {order.phone_number && (
                    <p className="text-sm text-gray-500 mb-2">Phone: {order.phone_number}</p>
                  )}
